...
async function buscarTicket(prefix) { const query = $(`${prefix}-folio`).value.trim(); $(`err-${prefix}-folio`).hidden = true; if (!query || !sheetUrl) return; try { const result = await callSheet({ action:'get', query }); if (!result.success) throw new Error('No se encontró ese ticket'); const data = result.data; $(`${prefix}-datos`).innerHTML = `<div class="folio-display"><strong>${escapeHtml(data.Cliente)}</strong> - ${escapeHtml(`${data['Tipo de equipo']} ${data.Marca} ${data.Modelo}`)}<br>Folio: ${escapeHtml(data.Folio)}<br>Teléfono: ${escapeHtml(data.Telefono)}<br>Recibido: ${escapeHtml(data['Fecha recibido'])} - Saldo pendiente: ${fmt(Number(data['Saldo pendiente']) || 0)}<br>Entregado: ${escapeHtml(data.Entregado)}</div>`; currentFolio = String(data.Folio); currentTicketData = data; const entregado = String(data.Entregado).toLowerCase() === 'si'; $('btnEntregar').hidden = entregado; $('ok-entregado').hidden = !entregado; if (entregado) $('ok-entregado').textContent = `Este equipo ya fue entregado el ${data['Fecha de entrega']}.`; } catch (error) { $(`err-${prefix}-folio`).hidden = false; $(`${prefix}-datos`).innerHTML = ''; $('btnEntregar').hidden = true; } }
