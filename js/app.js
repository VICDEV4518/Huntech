...
function login(event) {
  ...
  if (!user || user.password !== $('loginPassword').value) { $('loginError').hidden = false; $('loginForm').classList.remove('is-error'); void $('loginForm').offsetWidth; $('loginForm').classList.add('is-error'); return; }
  sessionStorage.setItem('huntech-user', username);
  $('loginError').hidden = true;
  showApp(username);
}

function togglePassword() { const input = $('loginPassword'); const visible = input.type === 'text'; input.type = visible ? 'password' : 'text'; $('passwordToggle').textContent = visible ? 'Mostrar' : 'Ocultar'; $('passwordToggle').setAttribute('aria-label', visible ? 'Mostrar contraseña' : 'Ocultar contraseña'); }
...
function renderPreview(ticket) { const saldo = ticket.monto - ticket.anticipo; $('r-preview').innerHTML = `<div class="ticket-outer"><div class="ticket-header"><div class="ticket-brand"><img class="ticket-logo" src="assets/logo-huntech.png" alt="Huntech"><div><div class="brand-name">HUNTECH</div><div class="sub2">Comprobante de servicio</div></div></div></div><div class="ticket-body"><div class="folio-pill"><span>Folio</span><strong>${escapeHtml(ticket.folio)}</strong></div><div class="ticket-meta"><div class="t-row"><span class="t-label">Fecha</span><span>${escapeHtml(ticket.fecha)}</span></div><div class="t-row"><span class="t-label">Cliente</span><span>${escapeHtml(ticket.cliente)}</span></div><div class="t-row"><span class="t-label">Equipo</span><span>${escapeHtml(`${ticket.tipo} ${ticket.marca} ${ticket.modelo}`)}</span></div></div><div class="t-section">Servicio</div>${ticket.conceptos.map((item) => `<div class="t-row concept"><span>${escapeHtml(item.desc)}</span><span>${fmt(item.monto)}</span></div>`).join('')}<div class="totales"><div class="t-row"><span>Monto total</span><span>${fmt(ticket.monto)}</span></div><div class="t-row"><span>Anticipo</span><span>${fmt(ticket.anticipo)}</span></div><div class="t-row saldo"><span>Saldo pendiente</span><span>${fmt(saldo)}</span></div></div><div class="footer-note"><span>Gracias por su preferencia.</span><small>Huntech · Servicio técnico</small></div></div></div><div class="download-wrap"><button type="button" class="primary-button" id="downloadButton">Descargar comprobante PDF</button></div>`; $('downloadButton').addEventListener('click', () => descargarPDF(currentTicketData)); }

async function buscarTicket(prefix) { const query = $(`${prefix}-folio`).value.trim(); $(`err-${prefix}-folio`).hidden = true; if (!query || !sheetUrl) return; try { const result = await callSheet({ action:'get', query }); if (!result.success) throw new Error('No se encontraron tickets con ese dato'); deliveryResults = result.data || []; renderDeliveryResults(); } catch (error) { $(`err-${prefix}-folio`).hidden = false; $(`${prefix}-datos`).innerHTML = ''; deliveryResults = []; } }
function renderDeliveryResults() { const visible = deliveryResults.filter((data) => deliveryFilter === 'todos' || (deliveryFilter === 'entregados' ? String(data.Entregado).toLowerCase() === 'si' : String(data.Entregado).toLowerCase() !== 'si')); $('e-datos').innerHTML = visible.length ? visible.map((data) => { const entregado = String(data.Entregado).toLowerCase() === 'si'; return `<article class="delivery-result"><div class="folio-display"><strong>${escapeHtml(data.Cliente)}</strong> - ${escapeHtml(`${data['Tipo de equipo']} ${data.Marca} ${data.Modelo}`)}<br>Folio: ${escapeHtml(data.Folio)}<br>Teléfono: ${escapeHtml(data.Telefono)}<br>Recibido: ${escapeHtml(data['Fecha recibido'])} - Saldo pendiente: ${fmt(Number(data['Saldo pendiente']) || 0)}<br>Estado: ${entregado ? `Entregado el ${escapeHtml(data['Fecha de entrega'])}` : 'Pendiente'}</div>${entregado ? '' : `<button type="button" class="primary-button deliver-result-button" data-folio="${escapeHtml(data.Folio)}">Marcar como entregado</button>`}</article>`; }).join('') : '<p class="hint">No hay tickets en este filtro.</p>'; document.querySelectorAll('.deliver-result-button').forEach((button) => button.addEventListener('click', () => marcarEntregado(button.dataset.folio))); }
async function marcarEntregado(folio) { const fechaEntrega = new Date().toLocaleDateString('es-MX'); const button = document.querySelector(`[data-folio="${CSS.escape(folio)}"]`); if (button) { button.disabled = true; button.textContent = 'Actualizando...'; } try { const result = await callSheet({ action:'deliver', folio, fechaEntrega }); if (!result.success) throw new Error(result.error || 'No se pudo actualizar'); deliveryResults = deliveryResults.map((data) => String(data.Folio) === String(folio) ? { ...data, Entregado:'Si', 'Fecha de entrega':fechaEntrega } : data); renderDeliveryResults(); $('ok-entregado').hidden = false; $('ok-entregado').textContent = `El ticket ${folio} fue marcado como entregado.`; } catch (error) { if (button) { button.disabled = false; button.textContent = 'Marcar como entregado'; } alert(`No se pudo actualizar: ${error.message}`); } }
